...
const PANEL_META = {
  request: {
    heading:    "Forgot your\npassword?",
    subheading: "No worries — enter your email and we'll send a secure reset link.",
    icon: (
  ...
  reset: {
    ...
    subheading: "Make it strong. You won't need to change it again for a while.",
    icon: (
      <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <>
      ...
      <div style={{ position:"relative",width:120,height:120,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:28 }}>
        {[1,2,3].map(i=>(
          <div key={i} style={{ position:"absolute",width:i*40,height:i*40,borderRadius:"50%",border:"1px solid rgba(255,255,255,0.15)",animation:`pulseRing ${1.5+i*0.4}s ${i*0.2}s ease-out infinite` }}/>
        ))}
        <div style={{ position:"relative",width:80,height:80,borderRadius:20,background:"rgba(255,255,255,0.18)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.25)",display:"flex",alignItems:"center",justifyContent:"center" }}>
          {meta.icon}
        </div>
      </div>

      <h2 style={{ fontFamily:"'Playfair Display',serif",fontWeight:900,fontSize:"clamp(28px,3.5vw,44px)",color:"#fff",lineHeight:1.1,marginBottom:16,whiteSpace:"pre-line" }}>
        {meta.heading}
      </h2>
      <p style={{ fontSize:14,color:"rgba(255,255,255,0.55)",fontWeight:300,lineHeight:1.7,marginBottom:36,maxWidth:340 }}>
        {meta.subheading}
      </p>
...
export default function ResetPasswordPage() {
  ...
  const handleSetPassword = async () => {
    ...
    if (!pwV.valid || pwV.strength < 2) {
      setPwError("Choose a stronger password — at least 8 characters with a number or symbol.");
      return;
    }
    const matchV = validatePasswordMatch(password, confirm);
    if (!matchV.valid) { setConfirmError(matchV.message); return; }
    ...
    try {
      ...
      setApiError(e.message ?? "Failed to update password. The link may have expired.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {step === "request" && (
        <div className="fu">
          <h1 style={{ fontFamily:"'Playfair Display',serif",fontWeight:900,fontSize:28,color:"#1A1412",marginBottom:8 }}>
            Reset your password
          </h1>
          <p style={{ fontSize:14,color:"#8B7355",marginBottom:28,lineHeight:1.6 }}>
            Enter the email address linked to your account. We'll send you a link.
          </p>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:18 }}>
            ...
            <Button variant="primary" fullWidth loading={loading} onClick={handleSendReset}>
              Send Reset Link
            </Button>

            <Button variant="ghost" fullWidth onClick={() => navigate("/login")} style={{ marginTop:-4 }}>
              ← Back to sign in
            </Button>
          </div>
          ...
          {/* Dev shortcut */}
          {import.meta.env.DEV && (
            <div style={{ marginTop:24,paddingTop:20,borderTop:"1.5px dashed #EDE4D8",textAlign:"center" }}>
              <p style={{ fontSize:11,color:"#8B7355",marginBottom:8 }}>Dev shortcut</p>
              <button onClick={() => setStep("reset")}
                style={{ fontSize:12,color:"#C5612C",fontWeight:600,background:"none",border:"1px dashed rgba(197,97,44,0.4)",borderRadius:8,padding:"8px 16px",cursor:"pointer",transition:"background 0.15s" }}
      ...
      {/* ── Step: sent ── */}
      {step === "sent" && (
        <div className="fu" style={{ textAlign:"center",display:"flex",flexDirection:"column",gap:20 }}>
          <div style={{ width:72,height:72,borderRadius:20,background:"#ECFDF5",border:"2px solid #10B981",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto" }}>
          ...
          <div>
            ...
            <p style={{ fontSize:14,color:"#5C4A3A",lineHeight:1.65,margin:0 }}>
              We've sent a reset link to <strong>{email || "your email"}</strong>.<br/>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
            <Button variant="secondary" fullWidth onClick={() => { setStep("request"); setEmail(""); }}>
      ...
      {step === "reset" && (
        <div className="fu">
          <div style={{ marginBottom:24 }}>
            <p style={{ fontSize:11,fontWeight:700,color:"#C5612C",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:8 }}>
              New Password
            </p>
            <h1 style={{ fontFamily:"'Playfair Display',serif",fontWeight:900,fontSize:28,color:"#1A1412",marginBottom:6 }}>
          ...
          {apiError && <Alert type="error" message={apiError} style={{ marginBottom:18 }} />}

          <div style={{ display:"flex",flexDirection:"column",gap:18 }}>
            <PasswordInput label="New Password" required showStrength
              value={password}
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
                {requirements.map(r=>(
                  <div key={r.label} style={{
                    ...
                    <div style={{ width:14,height:14,borderRadius:"50%",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",background:r.met?"#10B981":"#E8DDD4" }}>
                      {r.met && <svg width="8" height="8" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    </div>
                    {r.label}
                  </div>
                ))}
            ...
            <Button variant="primary" fullWidth loading={loading} onClick={handleSetPassword}>
              Update Password →
            </Button>
          </div>
        </div>
      ...
      {step === "success" && (
        <div className="fu" style={{ display:"flex",flexDirection:"column",gap:20 }}>
          ...
          <div style={{ textAlign:"center" }}>
            <h1 style={{ fontFamily:"'Playfair Display',serif",fontWeight:900,fontSize:28,color:"#1A1412",marginBottom:10 }}>
              Password updated!
            </h1>
            <p style={{ fontSize:14,color:"#5C4A3A",lineHeight:1.65,margin:0 }}>
          ...
          <p style={{ fontSize:12,color:"#8B7355",textAlign:"center",margin:0 }}>
            Having trouble?{" "}
            <a href="mailto:support@fabrentals.co.ke" style={{ color:"#C5612C",textDecoration:"none",fontWeight:600 }}>
              Contact support
            </a>
          </p>
        </div>
