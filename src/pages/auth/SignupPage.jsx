...
function Field({ label, error, helper, required, children }) {
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
      ...
      {!error && helper && <span style={{ fontSize:12,color:"#8B7355" }}>{helper}</span>}
    </div>
  );
}

// ── Text input ────────────────────────────────────────────────────────────────
function TInput({ type="text", value, onChange, placeholder, autoComplete, error, rightSlot, id }) {
...
function PwField({ label, value, onChange, error, autoComplete, showStrength, required }) {
      ...
      <div style={{ position:"relative" }}>
        ...
        <button type="button" onClick={() => setVisible(v=>!v)}
          style={{ position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",
            background:"none",border:"none",cursor:"pointer",color:"#8B7355",display:"flex",alignItems:"center",padding:0 }}>
...
function PhoneField({ value, onChange, error }) {
  const [focused, setFocused] = useState(false);
  return (
    <Field label="Phone Number" error={error} helper="Used for M-Pesa payment notifications">
      <div style={{ display:"flex",border:`1.5px solid ${error?"#DC2626":focused?"#C5612C":"#E8DDD4"}`,borderRadius:10,
        overflow:"hidden",background:"#fff",transition:"border-color 0.18s",
        boxShadow:focused?`0 0 0 3px ${error?"rgba(220,38,38,0.10)":"rgba(197,97,44,0.12)"}`:"none" }}>
        <div style={{ padding:"12px 12px",background:"#FAF7F2",borderRight:"1px solid #E8DDD4",
          fontSize:13,fontWeight:600,color:"#5C4A3A",whiteSpace:"nowrap",userSelect:"none" }}>
          🇰🇪 +254
        </div>
        <input type="tel" value={value} onChange={e=>onChange(e.target.value)}
          ...
          style={{ flex:1,padding:"12px 14px",border:"none",outline:"none",
            fontSize:14,color:"#1A1412",background:"transparent" }}
        />
      </div>
    </Field>
  );
...
function Steps({ step }) {
  return (
    <div style={{ display:"flex",alignItems:"center",gap:0,marginBottom:28 }}>
      {[{n:1,label:"Credentials"},{n:2,label:"Your Info"}].map((s,i) => (
        <div key={s.n} style={{ display:"flex",alignItems:"center" }}>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            <div style={{
              ...
              boxShadow:s.n===step?"0 2px 10px rgba(197,97,44,0.35)":"none",
            }}>
              {s.n<step
            ...
            <span style={{ fontSize:12,fontWeight:s.n===step?700:400,
              color:s.n<=step?"#C5612C":"#8B7355",transition:"color 0.25s" }}>
              {s.label}
            </span>
          </div>
          {i<1 && (
            <div style={{ width:32,height:2,background:step>1?"#C5612C":"#E8DDD4",
              margin:"0 8px",borderRadius:999,transition:"background 0.3s" }}/>
          )}
        </div>
      ))}
    </div>
...
// ── Primary button ────────────────────────────────────────────────────────────
function PBtn({ children, onClick, type="button", loading, fullWidth, variant="primary" }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={loading}
      ...
      {loading&&<div style={{ width:16,height:16,border:"2px solid rgba(255,255,255,0.35)",
        borderTopColor:"#fff",borderRadius:"50%",animation:"sspin 0.7s linear infinite" }}/>}
      {children}
    </button>
  );
}
...
export default function SignupPage() {
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await signUp({
      ...
      phone:    form.phone ? `+254${form.phone.replace(/\D/g,"").replace(/^0/,"")}` : undefined,
    });
    setLoading(false);
    if (error) {
    ...
    <div style={{ minHeight:"100dvh",display:"flex",fontFamily:"'DM Sans',system-ui,sans-serif",background:"#FAF7F2" }}>
      ...
      <div className="sp-left" style={{
        ...
        <div style={{ position:"absolute",top:-80,right:-80,width:280,height:280,
          borderRadius:"50%",background:"rgba(197,97,44,0.08)",pointerEvents:"none" }}/>
        <div style={{ position:"absolute",bottom:-60,left:-40,width:220,height:220,
        ...
        <div>
          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:56 }}>
            ...
            <span style={{ fontFamily:"'Playfair Display',serif",fontWeight:900,
              fontSize:18,color:"#fff",letterSpacing:"-0.02em" }}>fabRentals</span>
          </div>

          <h1 style={{ fontFamily:"'Playfair Display',serif",fontWeight:900,
            fontSize:"clamp(34px,3.5vw,52px)",color:"#fff",lineHeight:1.08,
            marginBottom:16,letterSpacing:"-0.02em" }}>
            Your Next<br/>Home Starts<br/>
          ...
          <p style={{ fontSize:14,color:"rgba(255,255,255,0.52)",lineHeight:1.75,
            ...
            Join thousands of Kenyans managing their rentals, paying bills, and staying connected — all in one place.
          </p>
            ...
            {FEATURES.map((f,i)=>(
              <div key={i} style={{ display:"flex",alignItems:"center",gap:12 }}>
                <div style={{ width:22,height:22,borderRadius:"50%",
                  background:"rgba(197,97,44,0.2)",border:"1px solid rgba(197,97,44,0.4)",
                  display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
                  <svg width="10" height="10" viewBox="0 0 12 12" fill="none">
                    <path d="M2 6l3 3 5-5" stroke="#C5612C" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
                <span style={{ fontSize:13,color:"rgba(255,255,255,0.68)",lineHeight:1.4 }}>{f}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"center",
        ...
        <div style={{ width:"100%",maxWidth:420,animation:"fadeUp 0.4s ease both" }}>
          ...
          {/* Mobile logo */}
          <div className="sp-mob-logo" style={{ alignItems:"center",gap:8,marginBottom:28 }}>
          ...
          <h1 style={{ fontFamily:"'Playfair Display',serif",fontWeight:900,
            fontSize:30,color:"#1A1412",marginBottom:6,letterSpacing:"-0.02em" }}>
            Create Account
          </h1>
            ...
            <div style={{ background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:10,
              ...
              <div style={{ flex:1 }}>
                ...
                {apiError.includes("already exists") && (
                  <Link to="/login" style={{ fontSize:13,color:"#C5612C",fontWeight:700,textDecoration:"none" }}>
                    Sign in instead →
                  </Link>
                )}
              </div>
          ...
          {step === 1 && (
            <div style={{ display:"flex",flexDirection:"column",gap:18 }}>
              <Field label="Email Address" error={errors.email} required>
                <TInput id="su-email" type="email" value={form.email}
              ...
              <PwField label="Password" value={form.password}
                onChange={e=>{set("password",e.target.value);setErrors(p=>({...p,password:null}));}}
                error={errors.password} autoComplete="new-password" showStrength required/>

              <PwField label="Confirm Password" value={form.confirm}
                onChange={e=>{set("confirm",e.target.value);setErrors(p=>({...p,confirm:null}));}}
                error={errors.confirm} autoComplete="new-password" required/>

              <PBtn fullWidth onClick={handleNext}>Continue →</PBtn>

              {/* Divider */}
              <div style={{ display:"flex",alignItems:"center",gap:12 }}>
                <div style={{ flex:1,height:1,background:"#E8DDD4" }}/>
                <span style={{ fontSize:12,color:"#8B7355",whiteSpace:"nowrap" }}>or sign up with</span>
                <div style={{ flex:1,height:1,background:"#E8DDD4" }}/>
              </div>
          ...
          {step === 2 && (
            <div style={{ display:"flex",flexDirection:"column",gap:18 }}>
              <Field label="Full Name" error={errors.full_name} required>
                <TInput id="su-name" value={form.full_name}
                  onChange={e=>{set("full_name",e.target.value);setErrors(p=>({...p,full_name:null}));}}
                  placeholder="e.g. Jane Wanjiku" autoComplete="name" error={errors.full_name}/>
              </Field>
              ...
              {/* Info box */}
              <div style={{ background:"#FFF5EF",border:"1px solid rgba(197,97,44,0.22)",
                borderRadius:12,padding:"14px 16px",display:"flex",gap:12 }}>
                <div style={{ width:32,height:32,borderRadius:"50%",
                  background:"rgba(197,97,44,0.12)",flexShrink:0,
                  display:"flex",alignItems:"center",justifyContent:"center" }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="#C5612C">
                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/>
                  </svg>
                </div>
                <div>
              ...
              <div style={{ display:"flex",alignItems:"flex-start",gap:10 }}>
                <input type="checkbox" id="su-agree" checked={form.agree}
                  onChange={e=>{set("agree",e.target.checked);setErrors(p=>({...p,agree:null}));}}
                  style={{ marginTop:2,width:16,height:16,accentColor:"#C5612C",cursor:"pointer",flexShrink:0 }}
                />
                <label htmlFor="su-agree" style={{ fontSize:13,color:"#5C4A3A",lineHeight:1.6,cursor:"pointer" }}>
                  I agree to the{" "}
              ...
              {errors.agree && (
                <span style={{ fontSize:12,color:"#DC2626",marginTop:-10 }}>{errors.agree}</span>
              )}

              {/* Buttons */}
              <div style={{ display:"flex",gap:10 }}>
                <PBtn variant="secondary" onClick={()=>{setStep(1);setApiError(null);}}>
                  ← Back
                </PBtn>
