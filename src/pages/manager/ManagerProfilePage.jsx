...
export default function ManagerProfilePage() {
  ...
  if (loading || !profile) {
    return (
      <DashboardLayout pageTitle="My Profile">
        <PageHeader
          title="My Profile"
          subtitle="Manage your manager account details"
        />
        <div
          style={{
            display: "flex",
  ...
  const handleSave = async () => {
    setProfError(null);
    if (!fullName.trim()) {
      setProfError("Full name is required.");
      return;
    }
    setProfSaving(true);
    try {
      ...
      setProfError(e.message ?? "Failed to save.");
    } finally {
      setProfSaving(false);
    }
  };
  ...
  const handleChangePw = async () => {
    ...
    if (!newPw || !v.valid || v.strength < 2)
      e.newPw = "Choose a stronger password (min 8 chars).";
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <Alert
            type="info"
            compact
            message="Keep your phone number current — it's used for urgent property notifications and M-Pesa confirmations."
          />
        </div>
      </div>
    </DashboardLayout>
