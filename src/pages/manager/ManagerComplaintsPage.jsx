...
function ComplaintRow({ complaint, onStatusChange }) {
  ...
  return (
    <div
      onClick={() => navigate(`/manage/complaints/${complaint.id}`)}
      style={{
        display: "flex",
    ...
    >
      ...
      <Avatar
        ...
        src={complaint.submitter?.avatar_url}
        size="sm"
      />
        ...
        {next && (
          <button
            ...
            onMouseOut={(e) => {
              ...
              e.currentTarget.style.color = "#C5612C";
            }}
          >
...
export default function ManagerComplaintsPage() {
  ...
  const TABS = [
    { value: "open", label: "Open", count: counts.open || undefined },
    {
      value: "in_progress",
      label: "In Progress",
      count: counts.in_progress || undefined,
    },
      ...
      >
        {[
          ...
          <StatsCard
            ...
            label={label}
            value={val}
            color={val > 0 && color !== "neutral" ? color : "neutral"}
            icon={
              <svg
                width="15"
                height="15"
      ...
      >
        {loading ? (
          ...
          <EmptyState
            icon="complaints"
            title="No complaints"
            description="All complaints matching the current filter will appear here."
          />
        ) : (
          filtered.map((c) => (
            <ComplaintRow
