...
export default function WorkerProfilePage() {
  ...
  const handleSaveProfile = async () => {
    setProfError(null);
    if (!fullName.trim()) { setProfError("Full name cannot be empty."); return; }
    setProfSaving(true);
    try {
  ...
  const handleChangePassword = async () => {
    ...
    if (!newPw || !v.valid) errs.newPw = "Choose a stronger password (min 8 chars, mix of characters).";
  ...
  return (
    <DashboardLayout pageTitle="My Profile">
      <PageHeader title="My Profile" subtitle="Update your personal details and account settings" />
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 280px", gap:24, alignItems:"start" }}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
          ...
          <div style={{ background:"#fff", borderRadius:16, border:"1px solid #EDE4D8", padding:"24px" }}>
            <h3 style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:17,
              color:"#1A1412", marginBottom:18 }}>Personal Information</h3>

            {profError && (
              <Alert type="error" message={profError} style={{ marginBottom:14 }} />
            )}
            ...
            <div style={{ display:"flex", justifyContent:"flex-end", marginTop:20 }}>
              <Button variant="primary" loading={profSaving} onClick={handleSaveProfile}>
                Save Changes
              </Button>
            </div>
          </div>
          ...
          <div style={{ background:"#fff", borderRadius:16, border:"1px solid #EDE4D8", padding:"24px" }}>
            <h3 style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:17,
              color:"#1A1412", marginBottom:18 }}>Change Password</h3>

            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <PasswordInput
                label="New Password" showStrength
                value={newPw}
          ...
          <div style={{ background:"#fff", borderRadius:16, border:"1px solid #EDE4D8", padding:"24px" }}>
            <h3 style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:17,
              color:"#1A1412", marginBottom:14 }}>Account Details</h3>
            {[
              ...
              <div key={k} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                padding:"10px 0", borderBottom: i < arr.length-1 ? "1px solid #F5EDE0" : "none" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          {/* Avatar card */}
          <div style={{ background:"#fff", borderRadius:16, border:"1px solid #EDE4D8",
            padding:"24px", textAlign:"center" }}>

            {/* Avatar with pencil overlay */}
            <div style={{ position:"relative", display:"inline-block", marginBottom:14 }}>
              <Avatar name={profile?.full_name} src={profile?.avatar_url} size={76}
                style={{ margin:"0 auto" }} />
              <button
            ...
            <input
              ref={avatarRef} type="file" accept="image/*"
              onChange={handleAvatar}
              style={{ display:"none" }}
            />

            <p style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:17,
              color:"#1A1412", margin:"0 0 4px" }}>{profile?.full_name}</p>
            <p style={{ fontSize:12, color:"#8B7355", margin:"0 0 12px" }}>{user?.email}</p>
            <Badge variant="worker" size="md" />

            {avatarLoading && (
              <p style={{ fontSize:11, color:"#C5612C", marginTop:10 }}>Uploading…</p>
            )}
          ...
          {/* Quick links */}
          <div style={{ background:"#fff", borderRadius:16, border:"1px solid #EDE4D8", padding:"16px 20px" }}>
            <h4 style={{ fontSize:13, fontWeight:700, color:"#1A1412", marginBottom:10 }}>Quick Links</h4>
            {[
              ["/worker",            "My Dashboard"    ],
              ...
              <a key={path} href={path}
                style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                  padding:"9px 0", borderBottom:"1px solid #F5EDE0", textDecoration:"none",
                  color:"#5C4A3A", fontSize:13, fontWeight:500 }}
