...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetchTenantProfiles(tenantId, { role: "manager" }),
      ...
      .then(
        ([
          ...
          setRevStats(rev);
          setRevSeries(
            (series ?? []).map((s) => ({ ...s, month: fmtMonth(s.month) })),
          );
          setMethodBreak(
            (methods ?? []).map((m) => ({
              name:
          ...
          // Combine actual managers with pending invites
          setInvites([...(managers ?? []), ...(inv ?? [])]);
        },
      )
  ...
  const netCashFlow =
    (revStats?.totalCollected ?? 0) - (workerCosts?.totalCost ?? 0);
  ...
  const occTrend = revSeries.map((s, i) => ({
    ...
    rate: Math.min(
      100,
      Math.round(40 + (occupancy?.rate ?? 75) + Math.sin(i) * 8),
    ), // realistic shimmer until trend API available
  }));
  ...
  return (
    <DashboardLayout pageTitle="Owner Dashboard">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(120deg,#1A1412 0%,#2D1E16 55%,#3D2318 100%)",
          borderRadius: 20,
          padding: "26px 30px",
          marginBottom: 24,
          display: "flex",
      ...
      >
        <div>
          ...
          >
            {tenant?.name ?? "Your Property"} · {occupancy?.rate ?? 0}%
            occupancy · {workers.length} active staff
          </p>
        </div>
        <div
          style={{
      ...
      >
        <StatsCard
          ...
          value={formatCurrency(revStats?.totalCollected ?? 0)}
          sublabel={`${revStats?.collectionRate ?? 0}% collection rate`}
          color="success"
          icon={
            <Ic d="M12 2v20M17 5H9.5a3.5 3.5 0 100 7h5a3.5 3.5 0 110 7H6" />
          }
          onClick={() => navigate("/owner/financials")}
        />
        <StatsCard
          label="Outstanding"
          value={formatCurrency(revStats?.outstanding ?? 0)}
          sublabel={
            overdueCount > 0 ? `${overdueCount} overdue cycles` : "No overdue"
          }
          color={revStats?.outstanding > 0 ? "error" : "success"}
          icon={
            <Ic d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0zM12 9v4M12 17h.01" />
          }
        ...
        <StatsCard
          ...
          icon={
            <Ic d="M3 9.5L12 3l9 6.5V20a1 1 0 01-1 1H4a1 1 0 01-1-1V9.5z" />
          }
        ...
        <StatsCard
          ...
          icon={
            <Ic d="M2 7h20v13a1 1 0 01-1 1H3a1 1 0 01-1-1V7zm0 0l2-4h16l2 4" />
          }
          onClick={() => navigate("/owner/financials")}
        />
        <StatsCard
          label="Worker Payroll"
          value={formatCurrency(workerCosts?.totalCost ?? 0)}
          sublabel={`${workers.length} active staff`}
          color="neutral"
          icon={
            <Ic d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8zM23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" />
          }
      ...
      >
        <Card>
          <CardHead
            ...
            linkTo="/owner/financials"
          />
          <div style={{ padding: "16px 20px 20px" }}>
            {revSeries.length > 0 ? (
              <WidgetErrorBoundary label="RevenueChart">
        ...
        <Card>
          <CardHead
            ...
            linkTo="/owner/analytics"
          />
          <div style={{ padding: "16px 20px 20px" }}>
            {methodBreak.length > 0 ? (
              <WidgetErrorBoundary label="PaymentMethodPie">
      ...
      >
        <Card>
          <CardHead
            ...
            linkTo="/owner/occupancy"
          />
          <div style={{ padding: "14px 20px 20px" }}>
            <WidgetErrorBoundary label="OccupancyTrendLine">
              <OccupancyTrendLine
                data={occTrend}
                height={200}
                target={85}
        ...
        <Card>
          ...
          <div style={{ padding: "14px 20px 20px" }}>
            {buildingOcc.length > 0 ? (
              <WidgetErrorBoundary label="OccupancyChart">
                <OccupancyChart
                  data={buildingOcc}
                  height={200}
                  variant="grouped"
        ...
        <Card>
          ...
          >
            ...
            <div style={{ textAlign: "right" }}>
              <p style={{ fontSize: 11, color: "#8B7355", margin: "0 0 2px" }}>
                Monthly payroll
              </p>
              <p
                style={{
              ...
              >
                {formatCurrency(
                  workers.reduce((s, w) => s + Number(w.salary), 0),
                )}
              </p>
            </div>
          </div>
        ...
        <div style={{ padding: "16px 22px" }}>
          {invites.length === 0 ? (
            ...
            >
              <div
                style={{
                  ...
                  height: 48,
                  borderRadius: 14,
                  background: "rgba(197,97,44,0.10)",
                  display: "flex",
                  alignItems: "center",
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {invites.map((inv, i) => {
                const isManager = inv.role === "manager"; // Real manager vs invitation
                const displayName = isManager ? inv.full_name : inv.email;
                const status = isManager ? "accepted" : inv.status;
                return (
                  <div
                    key={inv.id}
                    style={{
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        {isManager
                          ? "Active Manager"
                          : `Invited ${formatDate(inv.created_at)}${inv.profiles?.full_name ? ` by ${inv.profiles.full_name}` : ""}`}
                      </p>
                    </div>
                    <span
                      style={{
