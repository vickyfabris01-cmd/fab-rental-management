...
function StatTile({ label, value, sub, color = ACCENT, alert }) {
  return (
    <div style={{ background: alert ? "rgba(197,97,44,0.08)" : SURFACE, borderRadius:14,
      border: `1px solid ${alert ? "rgba(197,97,44,0.3)" : BORDER}`, padding:"16px 18px" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        <span style={{ width:8, height:8, borderRadius:"50%", background:color, display:"inline-block" }}/>
        {alert && <span style={{ fontSize:10, fontWeight:700, color:"#F59E0B",
          background:"rgba(245,158,11,0.12)", padding:"2px 7px", borderRadius:999 }}>{alert}</span>}
      </div>
      <p style={{ fontFamily:"'Playfair Display',serif", fontWeight:900, fontSize:28,
        color:TEXT, margin:"0 0 3px" }}>{value}</p>
...
function ActionBtn({ onClick, icon, title, variant = "neutral" }) {
  const colors = {
    approve:  { bg:"rgba(16,185,129,0.10)", hover:"rgba(16,185,129,0.22)", color:"#10B981",  border:"rgba(16,185,129,0.22)" },
    suspend:  { bg:"rgba(239,68,68,0.10)",  hover:"rgba(239,68,68,0.22)",  color:"#EF4444",  border:"rgba(239,68,68,0.22)"  },
    neutral:  { bg:"rgba(255,255,255,0.05)",hover:"rgba(255,255,255,0.1)", color:DIM,         border:"transparent" },
  };
  const c = colors[variant];
  return (
    <button onClick={onClick} title={title}
      style={{ width:28, height:28, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center",
        background:c.bg, color:c.color, border:`1px solid ${c.border}`, cursor:"pointer", transition:"all 0.15s" }}
...
function THead({ cols }) {
  return (
    <tr style={{ borderBottom:`1px solid ${BORDER}` }}>
      {cols.map(c => (
        <th key={c} style={{ padding:"9px 16px", textAlign:"left", fontSize:10, fontWeight:700,
          color:MUTED, textTransform:"uppercase", letterSpacing:"0.08em" }}>{c}</th>
      ))}
    </tr>
  );
}
...
export default function SuperAdminDashboard() {
  ...
  const handleAction = async (tenantId, action) => {
    ...
    try {
      ...
      toast.success(`Tenant ${action === "approve" ? "approved" : action === "suspend" ? "suspended" : "reactivated"}.`);
      load();
    } catch (e) {
  ...
  return (
    <AdminLayout>
      ...
      {/* ── KPI tiles ── */}
      <div className="afu1" style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:12, marginBottom:20 }}>
        <StatTile label="Active Tenants"    value={stats?.tenants?.active    ?? 0}
          sub={`${stats?.tenants?.pending ?? 0} pending`} color={ACCENT}
          alert={pendingCount > 0 ? `${pendingCount} pending` : null} />
        <StatTile label="Total Users"       value={(stats?.users?.total ?? 0).toLocaleString()}
          sub={`${stats?.users?.client ?? 0} residents`} color="#3B82F6" />
        <StatTile label="Managers"          value={stats?.users?.manager ?? 0}
          sub={`${stats?.users?.owner ?? 0} owners`} color="#8B5CF6" />
        <StatTile label="Suspended"         value={stats?.tenants?.suspended ?? 0}
          sub="tenant accounts" color="#EF4444" />
        {analytics?.mrr && (
          <StatTile label="Platform MRR"    value={`KES ${(analytics.mrr/1000).toFixed(0)}K`}
            sub="monthly recurring" color="#10B981" />
        )}
      </div>

      {/* ── Growth chart + System health ── */}
      <div className="afu2" style={{ display:"grid", gridTemplateColumns:"1fr 280px", gap:14, marginBottom:20 }}>
        ...
        <div style={{ background:SURFACE, borderRadius:16, border:`1px solid ${BORDER}`, padding:"18px 20px" }}>
          <p style={{ fontSize:10, fontWeight:700, color:ACCENT, textTransform:"uppercase",
            letterSpacing:"0.1em", margin:"0 0 3px" }}>Platform Growth</p>
          ...
          {growthData.length > 0
            ...
            : (
              ...
              <div style={{ height:180, display:"flex", alignItems:"center", justifyContent:"center",
                ...
                <p style={{ fontSize:12, color:MUTED }}>Growth data available via FastAPI</p>
              </div>
            )
          }
        </div>
        ...
        <div style={{ background:SURFACE, borderRadius:16, border:`1px solid ${BORDER}`, padding:"18px 20px" }}>
          <p style={{ fontSize:10, fontWeight:700, color:ACCENT, textTransform:"uppercase",
            letterSpacing:"0.1em", margin:"0 0 3px" }}>Infrastructure</p>
          <h3 style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:17,
            color:TEXT, marginBottom:14 }}>System Health</h3>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {[
              ...
              { label:"FastAPI",      status:"healthy", uptime:"99.95%", latency:"22ms" },
            ].map(s => {
              ...
              return (
                <div key={s.label} style={{ display:"flex", alignItems:"center", gap:10,
                  ...
                  <span style={{ fontSize:12, color: ok ? DIM : "#F59E0B", flex:1 }}>{s.label}</span>
                  <span style={{ fontSize:10, color:MUTED, fontFamily:"'DM Mono','Courier New',monospace" }}>
                    {s.latency}
                  </span>
                  <span style={{ fontSize:10, fontWeight:700, color: ok ? "#10B981" : "#F59E0B" }}>
                    {s.uptime}
                  </span>
                </div>
              );
      ...
      {/* ── Tenants table ── */}
      <div className="afu3" style={{ background:SURFACE, borderRadius:16, border:`1px solid ${BORDER}`, overflow:"hidden", marginBottom:20 }}>
        <div style={{ padding:"16px 20px", borderBottom:`1px solid ${BORDER}`,
          display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
          <div>
            <p style={{ fontSize:10, fontWeight:700, color:ACCENT, textTransform:"uppercase",
              letterSpacing:"0.1em", margin:"0 0 2px" }}>Management</p>
            <h3 style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:17, color:TEXT, margin:0 }}>
              All Tenants
            </h3>
          </div>
            ...
            {["all","active","pending","suspended"].map(s => (
              <button key={s} onClick={() => setFilterStatus(s)}
                ...
                {s === "pending" && pendingCount > 0 &&
                  <span style={{ marginLeft:5, background:"#F59E0B", color:"#0F0D0C", fontSize:9,
                    fontWeight:700, padding:"1px 5px", borderRadius:999 }}>{pendingCount}</span>}
              </button>
            ))}
            <Link to="/admin/tenants" style={{ fontSize:11, fontWeight:600, color:ACCENT,
              textDecoration:"none", marginLeft:4 }}>View all →</Link>
          </div>
        </div>

        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", minWidth:700 }}>
            <thead><THead cols={["Tenant","Owner","Status","Plan","Joined","Actions"]} /></thead>
            <tbody>
              {filtered.slice(0,10).map((t, i) => (
                <tr key={t.id} className="t-row"
                  style={{ borderBottom: i < Math.min(filtered.length,10)-1 ? `1px solid ${BORDER}` : "none" }}>
                  <td style={{ padding:"11px 16px" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <div style={{ width:30, height:30, borderRadius:8,
                        background:"rgba(197,97,44,0.15)", display:"flex", alignItems:"center",
                        justifyContent:"center", fontSize:12, fontWeight:700, color:ACCENT, flexShrink:0 }}>
                        {t.name.charAt(0)}
                      </div>
                      <div>
                        <p style={{ fontSize:13, fontWeight:600, color:TEXT, margin:0 }}>{t.name}</p>
                        <p style={{ fontSize:10, color:MUTED, margin:"1px 0 0",
                          fontFamily:"'DM Mono','Courier New',monospace" }}>{t.slug}</p>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding:"11px 16px", fontSize:12, color:DIM }}>{t.email ?? "—"}</td>
                  <td style={{ padding:"11px 16px" }}><TenantStatusBadge status={t.status} /></td>
                  <td style={{ padding:"11px 16px", fontSize:11, color:MUTED, textTransform:"capitalize" }}>
                    {t.plan ?? "basic"}
                  </td>
                  ...
                  <td style={{ padding:"11px 16px" }}>
                    <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                      <ActionBtn onClick={() => navigate(`/admin/tenants/${t.id}`)}
                        icon={<Ic d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8zM12 12a3 3 0 100-6 3 3 0 000 6z"/>}
                        title="View" variant="neutral" />
                      {t.status === "pending" && (
                        <ActionBtn onClick={() => handleAction(t.id,"approve")}
                          icon={<Ic d="M5 13l4 4L19 7"/>} title="Approve" variant="approve"
                          disabled={actioning === t.id} />
                      )}
                      {t.status === "active" && (
                        <ActionBtn onClick={() => handleAction(t.id,"suspend")}
                          icon={<Ic d="M18 6L6 18M6 6l12 12"/>} title="Suspend" variant="suspend"
                          disabled={actioning === t.id} />
                      )}
                      {t.status === "suspended" && (
                        <ActionBtn onClick={() => handleAction(t.id,"reactivate")}
                          icon={<Ic d="M5 13l4 4L19 7"/>} title="Reactivate" variant="approve"
                          disabled={actioning === t.id} />
                      )}
                    </div>
                  </td>
          ...
          {filtered.length === 0 && (
            <p style={{ fontSize:13, color:MUTED, textAlign:"center", padding:"32px" }}>No tenants found.</p>
          )}
        </div>
        ...
        <div style={{ padding:"10px 20px", borderTop:`1px solid ${BORDER}`,
          ...
          <Link to="/admin/tenants" style={{ fontSize:11, fontWeight:600, color:ACCENT, textDecoration:"none" }}>
      ...
      {/* ── User role breakdown ── */}
      {stats?.users && (
        <div className="afu4" style={{ background:SURFACE, borderRadius:16, border:`1px solid ${BORDER}`, padding:"18px 20px" }}>
          <p style={{ fontSize:10, fontWeight:700, color:ACCENT, textTransform:"uppercase",
            letterSpacing:"0.1em", margin:"0 0 3px" }}>Platform Users</p>
          <h3 style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:17,
            color:TEXT, marginBottom:14 }}>Users by Role</h3>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))", gap:10 }}>
            {[
              ...
              <div key={label} style={{ background:"rgba(255,255,255,0.03)", borderRadius:12,
                border:`1px solid ${BORDER}`, padding:"12px 14px" }}>
                <p style={{ fontSize:11, color:MUTED, margin:"0 0 5px" }}>{label}</p>
                <p style={{ fontFamily:"'Playfair Display',serif", fontWeight:900,
                  fontSize:22, color:TEXT, margin:0 }}>{count.toLocaleString()}</p>
