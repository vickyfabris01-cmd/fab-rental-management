...
export default function RoomFormModal({
  isOpen, onClose, room, buildingId, buildings = [], onSuccess,
}) {
  ...
  useEffect(() => {
    ...
    if (room) {
      ...
      setRoomImages((room.images ?? []).map(url => ({ url, isExisting: true })));
      setImageFiles([]);
    } else {
      setForm({
        roomNumber:    "",
  ...
  const handleImagePick = (e) => {
    const files = Array.from(e.target.files ?? []).slice(0, 6 - roomImages.length);
    setImageFiles(prev => [...prev, ...files]);
    files.forEach(file => {
      const reader = new FileReader();
      reader.onload = ev => setRoomImages(prev => [...prev, { url: ev.target.result, isNew: true, file }]);
  ...
  const validate = () => {
    ...
    if (!form.buildingId)
      e.buildingId  = "Please select a building or block.";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const existingUrls = roomImages.filter(i => i.isExisting).map(i => i.url);
      const newUrls = [];
      if (imageFiles.length > 0) {
        ...
        for (const file of imageFiles) {
          ...
          try {
            const url = await uploadFile(BUCKETS.PROPERTY_IMGS, path, file, { upsert: true });
            newUrls.push(url);
    ...
    <Modal
      ...
      footer={
        <>
          ...
          >
            {uploadLoading ? "Uploading images…" : isEdit ? "Save Changes" : "Create Room"}
          </Button>
        </>
      }
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        {noBuildingsYet && (
          <div style={{
            ...
            <p style={{ fontSize: 13, color: "#991B1B", margin: 0, lineHeight: 1.5 }}>
              You need to create a <strong>building or block</strong> first before adding rooms.
              Close this and click <strong>+ Building</strong>.
            </p>
          </div>
        )}
        ...
        <div>
          <SelectInput
            ...
            onChange={v => { set("buildingId", v); setErrors(p => ({ ...p, buildingId: null })); }}
            error={errors.buildingId}
            disabled={noBuildingsYet}
          />
          {!errors.buildingId && form.buildingId && (
            <p style={{ fontSize: 11, color: "#10B981", marginTop: 5, display: "flex", alignItems: "center", gap: 4 }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#10B981" strokeWidth="2.5" strokeLinecap="round">
                <path d="M20 6L9 17l-5-5"/>
              </svg>
              Room will be added to <strong>{buildings.find(b => b.id === form.buildingId)?.name}</strong>
            </p>
          )}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <Input
            label="Room Number / Label" required
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <Input
            label="Capacity (persons)" type="number" min="1" max="50" required
            value={form.capacity}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <Input
            label="Monthly Rent (KES)" type="number" min="1" required
          ...
          />
          <Input
            label="Semester Price (KES)" type="number" min="1"
        ...
        {/* Room Photos */}
        <div>
          <p style={{ fontSize: 13, fontWeight: 600, color: "#1A1412", marginBottom: 8 }}>
            Room Photos <span style={{ fontSize: 11, fontWeight: 400, color: "#8B7355" }}>(up to 6)</span>
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
            {roomImages.map((img, idx) => (
              <div key={idx} style={{ position: "relative", aspectRatio: "4/3", borderRadius: 10, overflow: "hidden", border: "1px solid #EDE4D8" }}>
                <img src={img.url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                <button type="button" onClick={() => removeRoomImage(idx)}
                ...
                {idx === 0 && <span style={{ position: "absolute", bottom: 4, left: 4, fontSize: 9, fontWeight: 700, background: "#C5612C", color: "#fff", padding: "2px 5px", borderRadius: 999 }}>COVER</span>}
              </div>
            ))}
            {roomImages.length < 6 && !noBuildingsYet && (
              <button type="button" onClick={() => imgRef.current?.click()}
                style={{ aspectRatio: "4/3", borderRadius: 10, border: "2px dashed #E8DDD4", background: "#FAF7F2",
                  cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center",
                  justifyContent: "center", gap: 4, color: "#8B7355" }}
          ...
          <input ref={imgRef} type="file" accept="image/jpeg,image/png,image/webp" multiple
        ...
        <div>
          <p style={{ fontSize: 13, fontWeight: 600, color: "#1A1412", marginBottom: 10 }}>
            Amenities <span style={{ fontSize: 11, fontWeight: 400, color: "#8B7355" }}>(select all that apply)</span>
          </p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: "#1A1412" }}>
            Description <span style={{ color: "#8B7355", fontWeight: 400 }}>(optional)</span>
          </label>
          <textarea
