...
export function BuildingFormModal({ isOpen, onClose, building, onSuccess }) {
  ...
  useEffect(() => {
    if (isOpen && building) {
      ...
      setImages((building.images ?? []).map(url => ({ url, isExisting: true })));
      setImageFiles([]);
    } else if (isOpen) {
  ...
  const handleImagePick = (e) => {
    ...
    newFiles.forEach(file => {
      ...
      reader.onload = ev => setImages(prev => [...prev, { url: ev.target.result, isNew: true, file }]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? "Edit Building" : "Add Building"} size="md"
      footer={<>
        <Button variant="secondary" onClick={onClose}>Cancel</Button>
        <Button variant="primary" loading={loading || uploadLoading} onClick={handleSubmit}>
          {uploadLoading ? "Uploading images…" : isEdit ? "Save Changes" : "Add Building"}
        </Button>
      </>}>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {/* Image Upload */}
        <div>
          <label style={{ fontSize: 13, fontWeight: 600, color: "#1A1412", display: "block", marginBottom: 8 }}>
            Building Photos <span style={{ color: "#8B7355", fontWeight: 400 }}>(up to 6 · JPEG/PNG/WebP)</span>
          </label>
              ...
              <div key={idx} style={{ position: "relative", aspectRatio: "4/3", borderRadius: 10, overflow: "hidden", border: "1px solid #EDE4D8" }}>
                ...
                {idx === 0 && (
                  <span style={{ position: "absolute", bottom: 4, left: 4, fontSize: 9, fontWeight: 700,
                    background: "#C5612C", color: "#fff", padding: "2px 6px", borderRadius: 999 }}>COVER</span>
                )}
              </div>
            ))}
            {images.length < 6 && (
              <button type="button" onClick={() => imgRef.current?.click()}
                style={{ aspectRatio: "4/3", borderRadius: 10, border: "2px dashed #E8DDD4",
                  background: "#FAF7F2", cursor: "pointer", display: "flex", flexDirection: "column",
                  alignItems: "center", justifyContent: "center", gap: 6, color: "#8B7355",
          ...
          <input ref={imgRef} type="file" accept="image/jpeg,image/png,image/webp"
            multiple onChange={handleImagePick} style={{ display: "none" }} />
