...
const ICONS = {
  ...
  billing:       "M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z M14 2v6h6 M16 13H8 M16 17H8 M10 9H8",
  payments:      "M3 10h18 M7 15h.01 M11 15h2 M5 5h14a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2z",
  ...
  residents:     "M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2 M23 21v-2a4 4 0 00-3-3.87 M16 3.13a4 4 0 010 7.75",
  workforce:     "M14.5 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V7.5L14.5 2z M10 13a2 2 0 104 0c0-1.1-.9-2-2-2a2 2 0 00-2 2z M8 17h.01 M12 17h.01 M16 17h.01",
  ...
  salary:        "M12 8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z M20.94 11A9 9 0 113 11",
  ...
  overview:      "M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6z M14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6z M4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2z M14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z",
  ...
  tenancies:     "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6",
...
function getNavItems(role) {
  if (role === "client") return [
    ...
    { type: "item",  label: "Billing & Invoices",  icon: "billing",       to: "/dashboard/billing" },
    { type: "item",  label: "Payments",            icon: "payments",      to: "/dashboard/payments" },
    { type: "item",  label: "Room Transfer",       icon: "transfer",      to: "/dashboard/transfer-request" },
    { type: "item",  label: "Complaints",          icon: "complaints",    to: "/dashboard/complaints" },
    { type: "item",  label: "Notifications",       icon: "notifications", to: "/dashboard/notifications" },
  ];

  if (role === "manager") return [
    { type: "item",  label: "Dashboard",     icon: "dashboard",  to: "/manage" },
    { type: "group", label: "Properties",    icon: "properties", children: [
      { label: "Buildings & Rooms", to: "/manage/properties" },
      { label: "Rental Requests",   to: "/manage/residents/requests" },
    ]},
    { type: "group", label: "Residents",     icon: "residents",  children: [
  ...
  if (role === "owner") return [
    ...
    { type: "item", label: "Occupancy Report", icon: "occupancy",   to: "/owner/occupancy" },
    { type: "item", label: "Financials",       icon: "financials",  to: "/owner/financials" },
  ...
  if (role === "worker") return [
    ...
    { type: "item", label: "Pay History", icon: "salary",     to: "/worker/payments" },
    { type: "item", label: "Attendance",  icon: "attendance", to: "/worker/attendance" },
  ];
  ...
  return (
    <div>
      <button
        ...
        onMouseOver={e => { if (!hasActive) e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
        onMouseOut={e  => { if (!hasActive) e.currentTarget.style.background = "transparent"; }}
      >
        ...
        {!collapsed && (
          <>
            ...
            <span style={{ transform: open ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.2s", opacity: 0.5 }}>
...
function NavItem({ item, collapsed }) {
  return (
    <NavLink
      ...
      onMouseOver={e => { if (!e.currentTarget.style.background.includes("0.12")) e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
      onMouseOut={e  => { if (!e.currentTarget.style.background.includes("0.12")) e.currentTarget.style.background = "transparent"; }}
    >
      {({ isActive }) => (
        <>
          {/* Active indicator bar */}
          {isActive && (
            <div style={{ position:"absolute",left:0,top:"20%",bottom:"20%",width:3,borderRadius:"0 3px 3px 0",background:"#C5612C" }}/>
          )}
...
// ─── Sidebar ─────────────────────────────────────────────────────────────────
function Sidebar({ role, navItems, collapsed, onSignOut, profile, tenant, logoUrl }) {
  ...
  return (
    <div style={{
      ...
      {/* Logo */}
      <div style={{ padding: collapsed ? "20px 0" : "20px 20px", display: "flex", alignItems: "center", justifyContent: collapsed ? "center" : "flex-start", flexShrink: 0 }}>
        {logoUrl ? (
          <img src={logoUrl} alt="logo" style={{ height: 28, width: "auto", maxWidth: 180, objectFit: "contain" }} />
        ) : collapsed ? (
          <img src={LogoIconSrc} alt="fabrentals" style={{ width: 28, height: 28 }} />
        ) : (
          <div>
            <img src={LogoIconSrc} alt="" style={{ width: 24, height: 24, display: "inline", verticalAlign: "middle", marginRight: 8 }} />
            <span style={{ fontFamily: "'Playfair Display', serif", fontWeight: 900, fontSize: 17, color: "#fff", verticalAlign: "middle" }}>
              fab<span style={{ color: "#C5612C" }}>rentals</span>
            </span>
            {tenant?.name && (
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginTop: 2, paddingLeft: 32, letterSpacing: "0.02em" }}>
                {tenant.name}
              </div>
            )}
          </div>
      ...
      <div style={{ height: 1, background: "rgba(255,255,255,0.07)", margin: "0 16px", flexShrink: 0 }} />

      {/* Nav */}
      <nav style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "12px 10px" }}>
        {navItems.map((item, i) => {
          if (item.type === "divider") return (
            <div key={i} style={{ height: 1, background: "rgba(255,255,255,0.07)", margin: "8px 4px" }} />
          );
      ...
      {/* Profile + Sign out */}
      <div style={{ padding: collapsed ? "14px 0" : "14px 10px", flexShrink: 0 }}>
        <NavItem item={{ label: "My Profile", icon: "profile", to: `/${role === "client" ? "dashboard" : role === "manager" ? "manage" : role}/profile` }} collapsed={collapsed} />
        <button
          onClick={onSignOut}
        ...
        >
          <span style={{ flexShrink: 0 }}><Icon d={ICONS.signout} size={17}/></span>
          {!collapsed && <span>Sign Out</span>}
        </button>

        {/* User strip */}
        {!collapsed && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 16, padding: "10px 14px", background: "rgba(255,255,255,0.04)", borderRadius: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(197,97,44,0.3)", border: "1.5px solid rgba(197,97,44,0.5)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              {profile?.avatar_url
                ? <img src={profile.avatar_url} alt="" style={{ width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover" }} />
                : <span style={{ fontSize: 11, fontWeight: 700, color: "#C5612C" }}>{initials}</span>
              }
            </div>
...
function Topbar({ onMobileMenuOpen, title }) {
  ...
  const notifPath = role === "client" ? "/dashboard/notifications" : role === "manager" ? "/manage/notifications" : `/${role}`;
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        <button
          ...
          style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: "#5C4A3A", display: "flex" }}
          className="topbar-burger"
        >
          <Icon d={ICONS.menu} size={20}/>
        </button>
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: 18, color: "#1A1412", margin: 0 }}>
          {title}
        </h1>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        {/* Bell */}
        <Link to={notifPath} style={{
          position: "relative", display: "flex", alignItems: "center", justifyContent: "center",
          width: 38, height: 38, borderRadius: 10,
          background: hasUnread ? "rgba(197,97,44,0.08)" : "transparent",
          border: hasUnread ? "1px solid rgba(197,97,44,0.2)" : "1px solid transparent",
          color: "#5C4A3A", textDecoration: "none",
          transition: "all 0.18s",
        }}
          onMouseOver={e => e.currentTarget.style.background = "rgba(197,97,44,0.08)"}
          onMouseOut={e  => e.currentTarget.style.background = hasUnread ? "rgba(197,97,44,0.08)" : "transparent"}
        >
        ...
        <div style={{
          ...
          cursor: "default",
          fontSize: 12, fontWeight: 700, color: "#C5612C",
        }}>
          {profile?.avatar_url
            ? <img src={profile.avatar_url} alt="" style={{ width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover" }} />
            : initials
      ...
      <div style={{ display: "flex", height: "100dvh", overflow: "hidden" }}>
        ...
        {mobileOpen && (
          <div
            ...
            style={{ position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 210, display: "flex" }}
          >
            <Sidebar
              role={role}
              navItems={navItems}
            ...
            <button
              onClick={() => setMobileOpen(false)}
              style={{ position:"absolute",top:16,right:-40,background:"rgba(255,255,255,0.15)",border:"none",borderRadius:"50%",width:32,height:32,cursor:"pointer",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center" }}
            >
